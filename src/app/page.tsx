...
const App: React.FC = () => {
  ...
  const [items, setItems] = useState<Item[]>([
...

    {
      ...
      name: "Wind Turbine",
      description:
        "A device that converts wind's kinetic energy into electrical power.",
      image:
        "https://miro.medium.com/v2/resize:fit:1400/1*pvT6DZFwNsjgWuGFglpO2w.png",
      type: "token",
      tokenBalance: 10,
    ...
    },
    {
      ...
      name: "Farmland Lease",
      description:
        "A contract allowing farmers to use the land for cultivation for a specified period, ensuring mutual benefit and sustainable practices.",
      image:
        "https://assets.isu.pub/document-structure/230211065358-42cae78b997f657f3d0a932f4475cfed/v1/bbecf0660491378a3b9e6fd18625fbfa.jpeg",
      type: "nft",
    },
    {
...

    {
      ...
      description:
        "A patent for an AI-based software optimizing energy usage in smart homes.",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcInmh4BHgK2neD0hvi1C72Tv7lM2IkCji8w&s",
      type: "token",
      tokenBalance: 15,
  ...
  const fetchNFTs = async () => {
    try {
      ...
      const { classes } = await qclient.cosmos.nft.v1beta1.classes()
      ...
      for (const c of classes) {
        ...
        const nftItems = result.nfts.map((nft) => {
          const { image, name, description } = fractionnft.v1.NFTData.decode(
            nft.data?.value!
          )
          return {
            name,
                      ...
                      {item.type === "token" ? (
                        <Button
                          ...
                          style={{
                            width: "90%",
                            background:
                              "linear-gradient(45deg, #FF69B4, #8A2BE2)", // Pink to Purple gradient
                            color: "#fff",
                          }}
                        >
                          Remint
